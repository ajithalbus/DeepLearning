...
class MalarSpider(CrawlSpider):
    name = 'malar'
    allowed_domains = ['eluthu.com']
    start_urls = ['https://eluthu.com/']
    rules = (
        Rule(LinkExtractor(allow="http://eluthu\.com/.+_.+"),
             callback='parse'),
    )
    def parse(self, response):
        ...
        f=open(filename, 'a+')
        #self.log('HERE')
        for line in response.css('#kavithai_all>p::text').extract():
            line=line.replace(',',u' ')
            line=_clean(line)
        ...
        for link in next_page:
            ...
            if link.startswith('https://eluthu'):
